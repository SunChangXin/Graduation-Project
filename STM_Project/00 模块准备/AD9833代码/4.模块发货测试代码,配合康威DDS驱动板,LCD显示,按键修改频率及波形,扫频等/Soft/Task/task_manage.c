...
void Set_PointFre(u32 Key_Value, u8* Task_ID)
{
	...
	//其他处理
	if(Task_Index != 3) SweepFlag=0;//
	else SweepFlag=1;
	//固定显示
	LCD_Show_CEStr(0,0,"9833");
}
//任务
...
void Task0_PointFre(u32 Key_Value)//正弦波 (10M) 0-100 000 000
{
	...
	if(Key_Value != K_NO)
	{
		...
		//数据处理写入
		AD9833_SetFrequencyQuick(SinFre,AD9833_OUT_SINUS);
...
void Task1_Square(u32 Key_Value)//方波 300k
{
	...
	if(Task_First)
	{
		Task_First = 0;
		Key_Value = K_2_S;
		sprintf(fre_buf, "%5d", SquareFre);//第一次 进入
	...
	if(Key_Value != K_NO)
	{
		...
		if(SquareFre>100000000) SquareFre=100000000;//数据限制
		if(SquareFre<0) SquareFre=0;
		sprintf(fre_buf, "%9d", SquareFre);//字符转换
		//显示
		sprintf(showstr, "%9d", SquareFre);//字符转换
		fre_buf_change(showstr);//fre_buf当中 ‘ '->'0'
		Copybuf2dis(showstr, display, P_Index, 0, 1);
		OLED_ShowString(64-4*11, 3, display);
		LCD_Show_CEStr(64-4*11+9*8,3,"Hz");
		//数据处理写入
		AD9833_SetFrequencyQuick(SquareFre,AD9833_OUT_MSB);
...
void Task2_Triangular(u32 Key_Value)//三角波 1M
{
	...
	if(Task_First)
	{
		Task_First = 0;
		Key_Value = K_2_S;
		sprintf(fre_buf, "%5d", TriFre);//第一次 进入
	...
	if(Key_Value != K_NO)
	{
		//判断
		P_Index = P_Index%9;//数据位数
		TriFre = atol(fre_buf);//字符转换数字，判断上下限
		if(TriFre>100000000) TriFre=100000000;//数据限制
		if(TriFre<0) TriFre=0;
		sprintf(fre_buf, "%9d", TriFre);//字符转换
		//显示
		sprintf(showstr, "%9d", TriFre);//字符转换
		fre_buf_change(showstr);//fre_buf当中 ‘ '->'0'
		Copybuf2dis(showstr, display, P_Index, 0, 1);
		OLED_ShowString(64-4*11, 3, display);
		LCD_Show_CEStr(64-4*11+9*8,3,"Hz");
		//数据处理写入
		AD9833_SetFrequencyQuick(TriFre,AD9833_OUT_TRIANGLE);
