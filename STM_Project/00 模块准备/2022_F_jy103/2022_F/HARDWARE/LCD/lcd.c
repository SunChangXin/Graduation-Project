...
void LCD_Fast_DrawLine(u16 x1, u16 y1, u16 x2, u16 y2,u16 color)
{
	u16 t; 
	int xerr=0,yerr=0,delta_x,delta_y,distance; 
	int incx,incy,uRow,uCol; 
...
void LCD_ShowFloat5(u16 x,u16 y,float num,u8 len,u8 size)
{ 
	      int a;
	      a=num*100;
				if(a<0)
				...
				LCD_ShowxNum(x+size/2,y,a/100,6,size,0);//若是想显示12345.6的话，就是4
				
				LCD_ShowChar(x+size/2*6,y,'.',size,0);//分别乘6 7 8
								
				LCD_ShowxNum(x+size/2*7,y,(a-(a/100)*100)/10,1,size,0);
				
				LCD_ShowxNum(x+size/2*8,y,(a-(a/100)*100-((a-(a/100)*100)/10)*10),1,size,0);
}
